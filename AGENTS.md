...
**Task tags:** `tasks.tags text[]` (lowercase labels via `lib/taskTags.ts`). Edit in `TaskDetailModal`; filter on `/tasks` with `TaskTagFilter` (note action items hidden while a tag filter is active).
