...
**Assistant roadmap (owner reprioritized 2026-05):** B0 temperament ✅ → B meeting lifecycle ✅ → C delegation (slice 1 ✅) → **D capacity** ✅ → **E focus stack** ✅ (slice 1: `ExecutiveFocusStack` + `profiles.focus_queue`) → E decisions (full queue) → F capture/evening → A proactive email **deferred** (owner keeps app open). Document shifts here when order changes.
