...
| `events` | Calendar entries | `source` ('manual' \| 'outlook_ics'), `start_at`, `end_at`, recurrence fields, `outlook_source_key` (stable imported occurrence identity), `outlook_cancelled_at` (soft retirement preserves history), `prep_required`, `allow_back_to_back`, `debrief_required` (assistant temperament; Outlook rows flags-only on edit) |
...
**Agent-first roadmap (owner reprioritized 2026-08):** Today command center ✅ → deadline/review-date split ✅ → audited writes and activity ✅ → persisted briefings ✅ → live focus-plan sync ✅ → hosted OAuth MCP ✅ → weekday morning catch-up ✅ → evening closeout/open-loop capture → **Notes 2.0 (in progress: meeting-triage inbox, workstream lens, iterative migration, note audit, safe audited context append)** → richer Today context. Proactive email remains deferred; the owner keeps the app open. Document shifts here when order changes.
...
`2026-09-21_053_safe_calendar_sync.sql`; enable the Supabase OAuth server,
...
- **Agent approval is not connection health.** Profile's setup URL uses `VITE_MCP_PUBLIC_URL` (defaults to the canonical Vercel `/mcp`), never the internal Supabase URL. Grants are matched to owner-scoped `agent-connections` metadata by client ID, not name; display local approval/revocation and `last_used_at`, never infer a live connection or delete same-name clients. The function's authenticated `list` action returns only `oauth_client_id,last_used_at,revoked_at`; table access remains closed. Missing metadata displays unknown status while keeping grant revocation available. Deploy `agent-connections` with the frontend; no migration. Verify with `npm run test:connections`.
...
- **Outlook sync (migration 053):** `get_workspace_context` refreshes the published feed before reading events, using the same `_shared/outlookCalendar.ts` importer as the manual button. Requires an active write-enabled connection; read-only calls return saved events and `calendarSync.status = not_authorized`. Failures retain cached data and report last success explicitly; never infer free time from an unverified calendar. Outlook publication can still lag edits.
- **Stable calendar history:** service-only `apply_outlook_calendar_snapshot` atomically reconciles UID + original recurrence ID (`events.outlook_source_key`), preserves IDs/assistant flags/FKs, moves linked note/debrief occurrence keys on reschedule, updates last success and writes a `calendar_sync` audit. Missing occurrences are soft-retired with `outlook_cancelled_at`; active readers filter them, history lookups do not. Audit failure rolls back the entire import. Undo directs the user to change Outlook and resync. Never restore delete/reinsert syncing. The window is previous Monday through three Mondays after this week (covers the context's next 14 days); older history is retained. Legacy rows adopt identity only on an unambiguous exact slot/title/duration match; already-renamed/moved legacy meetings cannot be reliably reattached automatically.
- **Calendar safety:** bounded fetch, host-checked redirects, complete-feed parsing, recurrence limits and transactional import fail closed. Same-slot distinct UIDs remain separate. Recurrence exceptions override their master. Tests use real migrations in isolated PGlite (`npm run test:calendar`), not production. Apply 053 before deploying updated functions/frontend; no scheduler change is required.
